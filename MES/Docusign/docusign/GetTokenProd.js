...
const DOCUSIGN_IMPERSONATED_USER_GUID = 'd2f8e899-1d8c-47dd-95a9-817d86781691'; // API Username
const DOCUSIGN_PRIVATE_KEY_PATH = path.join(baseDirPath, 'private_key_prod.pem'); 
...
const jwtClaims = {
    ...
    scope: 'signature impersonation' // Scopes requested
};
...
async function getAccessToken(jwtToken) {
    const url = 'https://account.docusign.com/oauth/token';
...
(async () => {
    try {
        console.log('\n1. Get Consent URL');
        console.log(
            `https://account.docusign.com/oauth/auth?response_type=code&scope=signature%20impersonation&client_id=${DOCUSIGN_INTEGRATION_KEY}&redirect_uri=https://example.com`
        );
